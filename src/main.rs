...
fn main() {
    ...
    println!("libusb library:");
    println!("Can detach the kernel driver {}",context.supports_detach_kernel_driver());
    println!("Has HID access {}",context.has_hid_access());
    println!("Has hotplug support {}",context.has_hotplug());
    println!("Has capability {}",context.has_capability());

    // List of devices
    for mut device in context.devices().unwrap().iter() {
        let timeout = Duration::from_secs(1);
        // Device descriptor for device
